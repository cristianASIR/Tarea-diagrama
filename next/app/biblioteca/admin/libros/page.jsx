        ...
        <div className="container mx-auto px-4">
            ...
            <div className="mt-8">
                ...
                <ul className="space-y-4">
                    {libros.map((libro) => (
                        <li key={libro.id} className="flex items-center justify-between border p-4
   rounded">
                            <div>
                                <h3 className="text-xl">{libro.titulo}</h3>
                                <p className="text-gray-600">{libro.autor?.nombre && `Autor: $
   {libro.autor.nombre}`}</p>
                            </div>
                            <div>
                                <button onClick={() => handleDeleteLibro(libro.id)}
